...
const ArticleCard = ({ path = "/", article }: any) => {
  return (
    <Link href={path}>
      <article className="flex flex-col space-y-2 p-4 rounded-md border border-white/15 h-full max-w-[350px]  w-full">
        <div className="h-[17rem] overflow-hidden">
          <Image
            src={article?.image || "/assets/images/bento-img1.webp"}
        ...
        <div className="flex flex-col justify-between flex-1">
          ...
          <p className="line-clamp-1 text-gray9 text-sm font-normal flex-1 overflow-hidden">
            {article?.subtitle || "test"}
          </p>
          <p className="text-sm text-gray9 font-normal">
            {new Date(article?._creationTime || "1h ago").toLocaleDateString()}
          </p>
        </div>
      </article>
    </Link>
